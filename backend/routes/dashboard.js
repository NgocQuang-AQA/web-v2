...
export function createDashboardRouter({ filesRepo, testRunsRepo }) {
  ...
  router.get("/stats", async (req, res) => {
    try {
      ...
      const visits = await filesRepo.col("log-preview").countDocuments({ message: "Page View" });

      // 2. Top User
      const topUserAgg = await filesRepo.col("log-preview").aggregate([
      ...
      const vocUsage = {
        day: await filesRepo.col("log-preview").countDocuments({ source: "HelperVoc", createdAt: { $gte: startOfDay.toISOString() } }),
        week: await filesRepo.col("log-preview").countDocuments({ source: "HelperVoc", createdAt: { $gte: startOfWeek.toISOString() } }),
        month: await filesRepo.col("log-preview").countDocuments({ source: "HelperVoc", createdAt: { $gte: startOfMonth.toISOString() } }),
      };
      ...
      const testRuns = {
        day: await testRunsCol.countDocuments({ createdAt: { $gte: startOfDay.toISOString() } }),
        week: await testRunsCol.countDocuments({ createdAt: { $gte: startOfWeek.toISOString() } }),
        month: await testRunsCol.countDocuments({ createdAt: { $gte: startOfMonth.toISOString() } }),
      };

      // 5. Latest Test Run - from test-runs, order by startTime desc
      const latestAgg = await testRunsCol.aggregate([
        { $addFields: { _stDate: { $convert: { input: "$startTime", to: "date", onError: null, onNull: null } } } },
        { $sort: { _stDate: -1 } },
        { $limit: 1 }
      ]).toArray();
      ...
      for (const s of sources) {
         ...
         const latestProjectAgg = await testRunsCol.aggregate([
           { $match: { project: s.key } },
           { $addFields: { _stDate: { $convert: { input: "$startTime", to: "date", onError: null, onNull: null } } } },
           { $sort: { _stDate: -1 } },
           { $limit: 1 }
