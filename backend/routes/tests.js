...
export function createTestsRouter({ filesRepo }) {
  ...
  router.get('/runs', async (req, res) => {
    ...
    const totalAgg = await filesRepo
      .col('test-runs')
      .aggregate([...pipelineBase, { $count: 'n' }])
      .toArray()
  ...
  router.get('/cases', async (req, res) => {
    ...
    if (status) {
      ...
      if (s === 'SUCCESS') {
        ...
        match.status = { $nin: ['SUCCESS', 'FAILURE'] }
      } else {
    ...
    const sort =
      String(sortBy) === 'duration' ? { _dur: dir } : { _tiDate: dir }
    const pipelineBase = [
      {
        $addFields: {
          _tiDate: {
          ...
          _dur: {
            $convert: { input: '$duration', to: 'int', onError: 0, onNull: 0 },
          },
        },
      },
      { $match: match },
    ...
    for (const it of arr) {
      ...
      if (!second) {
        ...
        if (
          !tree[top] ||
          typeof tree[top] !== 'object' ||
          Array.isArray(tree[top])
        )
          tree[top] = {}
  ...
  router.get('/steps', async (req, res) => {
    ...
    const pipeline = [
      {
        $addFields: {
          _order: {
            $convert: { input: '$stepOrder', to: 'int', onError: 0, onNull: 0 },
          },
        },
      },
      { $match: match },
    ...
    function buildCurl(reqObj) {
      ...
      if (method && method !== 'GET')
        parts.push(`--request ${safeQuote(method)}`)
