...
export function createFilesRouter({ filesRepo, summarizeDir }) {
  ...
  const dlog = (...args) => {
    if (isDebug) console.log('[files]', ...args)
  }
  ...
  function resolveDirForCollection(collection, rawDir) {
    ...
    const cnRootEnv = toPosix(
      process.env.SERENITY_HISTORY_DIR_CN ||
        (qaRootEnv ? qaRootEnv.replace('global-qa', 'global-cn') : '')
    )
...
export function createFilesStaticRouter({ filesRepo }) {
  ...
  const isDebug =
    ...
    String(process.env.FILES_DEBUG || '').toLowerCase() === 'true'
  const dlog = (...args) => {
    if (isDebug) console.log('[files-static]', ...args)
  }
