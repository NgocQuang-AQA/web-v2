...
const isDebug =
  String(process.env.FILES_DEBUG || '').toLowerCase() === '1' ||
  String(process.env.FILES_DEBUG || '').toLowerCase() === 'true'
const dlog = (...args) => {
  if (isDebug) console.log('[scanner]', ...args)
}
...
async function listDirs(dir) {
  try {
    ...
    const stats = await Promise.all(
      dirs.map(async (d) => ({ d, st: await fs.stat(d) }))
    )
...
function extractItemsFromJson(data) {
  ...
  if (data && typeof data === 'object' && !Array.isArray(data)) {
    const hasTestCaseFields =
      data.name || data.title || data.result || data.status || data.testCase
  ...
  for (const arr of candidates) {
    for (const obj of arr) {
      ...
      const name =
        obj.name || obj.title || obj.testCase || obj.id || obj.displayName
      const status = mapStatus(
        obj.status || obj.result || obj.outcome || obj.testResult
      )
      const suite =
        obj.suite ||
        obj.userStory ||
      ...
      const durationMs = toDurationMs(
        obj.durationMs ||
          obj.duration ||
          obj.durationInMilliseconds ||
          obj.durationInMillis ||
          obj.testDuration
      )
      ...
      const finishedAt = toDate(
        obj.finishedAt || obj.endTime || obj.stop || obj.testEndTime
      )
      if (!name && !suite && status === 'unknown') continue
      items.push({
        name: name || String(status || 'unknown'),
        status,
        suite,
        durationMs,
        startedAt,
...
async function scanSerenityLatest(testRunId, rootDirOverride) {
  const defaultRoot =
    process.env.SERENITY_REPORT_ROOT ||
    process.env.SERENITY_HISTORY_DIR ||
    path.resolve(
      process.env.HOME || '',
      'gz-project',
      'global-qa',
  ...
  try {
    ...
    for (const f of targets) {
      ...
      for (const it of items) {
        ...
        const t = it.finishedAt
          ? new Date(it.finishedAt).getTime()
          : it.startedAt
            ? new Date(it.startedAt).getTime()
            : 0
    ...
    if (indexHtmlText) {
      ...
      if (nearResultIdx >= 0) {
        const windowText = indexHtmlText.slice(
          nearResultIdx,
          Math.min(indexHtmlText.length, nearResultIdx + 20000)
        )
        const dataRe =
          /data\s*:\s*\[(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\]/i
      ...
      if (!usedIndexCounts && tMatch) {
        ...
        while ((rm = rowRe.exec(featureText)) !== null) {
          ...
          const numericCells = Array.from(
            row.matchAll(/<td>\s*(\d+)\s*<\/td>/g)
          )
          const scenarios =
            numericCells.length >= 3
              ? Number(numericCells[2][1] || 0)
              : numericCells.length
                ? Number(numericCells[0][1] || 0)
                : 0
          const percents = Array.from(
            block.matchAll(
              /style=\"width:\s*([0-9.]+)%\;\s*background-color:\s*(#[0-9a-fA-F]{6})\"/g
            )
          ).map((m) => ({ pct: Number(m[1] || 0), color: m[2] }))
          let sPct = 0,
            fPct = 0,
            ePct = 0
