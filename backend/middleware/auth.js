...
export function createTokenService(secret) {
  ...
  const sign = (payload) => {
    ...
    const sig = crypto.createHmac("sha256", sec).update(data).digest("base64url");
    return `v1.${data}.${sig}`;
  };
  const verify = (token) => {
    try {
      ...
      const data = parts[1];
      const sig = parts[2];
      const expected = crypto.createHmac("sha256", sec).update(data).digest("base64url");
...
export function createAuthMiddleware(secret) {
  ...
  const requireAuth = (allowedRoles) => {
    return (req, res, next) => {
      ...
      if (Array.isArray(allowedRoles) && allowedRoles.length > 0) {
        if (!allowedRoles.includes(u.role)) return res.status(403).json({ message: "forbidden" });
      }
      next();
    };
  };
  const login = async (username, password) => {
    ...
    return { token, role: user.role, username: user.username, permissions, menus };
  };
