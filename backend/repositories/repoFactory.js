...
class ReportsRepoMongo {
  ...
  async suites({ from, to, page = 1, pageSize = 20 } = {}) {
    ...
    return agg.map((x) => {
      ...
      const status =
        x.failed === 0 ? 'passed' : x.passed === 0 ? 'failed' : 'partial'
      return {
        id: String(x._id || ''),
        name: String(x._id || ''),
        status,
    ...
    return agg.map((x) => ({
      ...
      trendMs:
        x.lastSeen && x.firstSeen
          ? new Date(x.lastSeen).getTime() - new Date(x.firstSeen).getTime()
          : 0,
    }))
  }
  ...
  async historySessions({ from, to, page = 1, pageSize = 20 } = {}) {
    ...
    return agg.map((x) => {
      ...
      return {
        ...
        percent,
        durationMs: x.durationMs || 0,
        suiteCount: Array.isArray(x.suiteSet)
          ? x.suiteSet.filter(Boolean).length
          : 0,
        startedAt: x.startedAt || null,
...
class ReportsRepoMemory {
  ...
  async count(filter) {
    return this.items.filter(
      (i) => !filter || Object.keys(filter).every((k) => i[k] === filter[k])
    ).length
  }
  async find(filter, { page = 1, pageSize = 20 } = {}) {
  ...
  async stats({ from, to } = {}) {
    ...
    const items = this.items.filter(
      (i) => (!fromD || i.createdAt >= fromD) && (!toD || i.createdAt <= toD)
    )
    ...
    const flakyCount = Array.from(byName.values()).filter(
      (s) => s.has('passed') && s.has('failed')
    ).length
    return {
      successRate,
      failedCount,
      flakyCount,
      totalRuntimeMinutes: Math.round((counts.durationMs || 0) / 60000),
    }
  }
  async suites({ from, to, page = 1, pageSize = 20 } = {}) {
    const fromD = from ? new Date(from) : null
    ...
    for (const i of items) {
      ...
      x.startedAt =
        x.startedAt && i.startedAt
          ? x.startedAt < i.startedAt
            ? x.startedAt
            : i.startedAt
          : x.startedAt || i.startedAt
      x.finishedAt =
        x.finishedAt && i.finishedAt
          ? x.finishedAt > i.finishedAt
            ? x.finishedAt
            : i.finishedAt
          : x.finishedAt || i.finishedAt
    ...
    const arr = Array.from(bySuite.entries()).map(([k, v]) => {
      ...
      const status =
        v.failed === 0 ? 'passed' : v.passed === 0 ? 'failed' : 'partial'
      return {
        id: k,
        name: k,
        status,
  ...
  async flaky({ from, to, page = 1, pageSize = 20 } = {}) {
    ...
    const byName = new Map()
    for (const i of items) {
      const x = byName.get(i.name) || {
        suites: new Set(),
        statuses: new Set(),
        failures: 0,
        lastSeen: i.finishedAt || i.updatedAt,
        firstSeen: i.startedAt || i.createdAt,
      }
      x.suites.add(i.suite)
      x.statuses.add(i.status)
      if (i.status === 'failed') x.failures++
      x.lastSeen =
        x.lastSeen && i.finishedAt
          ? x.lastSeen > i.finishedAt
            ? x.lastSeen
            : i.finishedAt
          : x.lastSeen || i.finishedAt || i.updatedAt
      x.firstSeen =
        x.firstSeen && i.startedAt
          ? x.firstSeen < i.startedAt
            ? x.firstSeen
            : i.startedAt
          : x.firstSeen || i.startedAt || i.createdAt
      byName.set(i.name, x)
    }
    const arr = Array.from(byName.entries())
      ...
      .map(([k, v]) => ({
        id: k,
        title: k,
        suite: Array.from(v.suites)[0] || '',
        failures: v.failures,
        lastSeen: v.lastSeen || null,
        trendMs:
          v.lastSeen && v.firstSeen
            ? new Date(v.lastSeen).getTime() - new Date(v.firstSeen).getTime()
            : 0,
      }))
    arr.sort(
      (a, b) =>
        (new Date(b.lastSeen).getTime() || 0) -
        (new Date(a.lastSeen).getTime() || 0)
    )
    const start = (Number(page) - 1) * Number(pageSize)
    return arr.slice(start, start + Number(pageSize))
  }
  ...
  async historySessions({ from, to, page = 1, pageSize = 20 } = {}) {
    ...
    for (const i of items) {
      ...
      if (saMs != null && (curSaMs == null || saMs < curSaMs))
        x.startedAt = new Date(saMs)

      const faMs = toMs(i.finishedAt || i.updatedAt)
      const curFaMs = toMs(x.finishedAt)
      if (faMs != null && (curFaMs == null || faMs > curFaMs))
        x.finishedAt = new Date(faMs)
      if (i.suite) x.suiteSet.add(i.suite)

      const caMs = toMs(i.createdAt)
      const curCaMs = toMs(x.createdAt)
      if (caMs != null && (curCaMs == null || caMs < curCaMs))
        x.createdAt = new Date(caMs)

      const uaMs = toMs(i.updatedAt)
      const curUaMs = toMs(x.updatedAt)
      if (uaMs != null && (curUaMs == null || uaMs > curUaMs))
        x.updatedAt = new Date(uaMs)
    ...
    arr.sort(
      (a, b) =>
        (new Date(b.finishedAt || b.updatedAt || 0).getTime() || 0) -
        (new Date(a.finishedAt || a.updatedAt || 0).getTime() || 0)
    )
    const start = (Number(page) - 1) * Number(pageSize)
    return arr.slice(start, start + Number(pageSize))
  }
...
class FilesRepoMemory {
  ...
  async count(collection, filter = {}) {
    ...
    const toD = to ? new Date(to) : null
    const nameLower =
      typeof name === 'string' && name ? String(name).toLowerCase() : ''
    const getMs = (v) => {
  ...
  async findById(collection, id) {
    return (
      this.list(collection).find((i) => i.id === id || i._id === id) || null
    )
  }
