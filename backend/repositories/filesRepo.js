...
class FilesRepoMongo {
    ...
    const pipeline = [
      {
        $addFields: {
          _tiDate: {
            $convert: { input: "$time_insert", to: "date", onError: null, onNull: null },
          },
        },
      },
      { $match: match },
  ...
  async find(collection, { page = 1, pageSize = 20, from, to, sortBy = "time_insert", order = "desc", name, key } = {}) {
    ...
    const pipeline = [
      {
        $addFields: {
          _tiDate: { $convert: { input: "$time_insert", to: "date", onError: null, onNull: null } },
          _sizeNum: { $convert: { input: "$size", to: "int", onError: 0, onNull: 0 } },
        },
      },
      { $match: match },
      { $sort: sort },
