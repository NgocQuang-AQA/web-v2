              ...
              {cols.map((k) => (
                <td key={k} className={`px-3 py-2 ${k === 'modeName' && tab !== 'gs' && shouldHighlightMode(row as Record<string, unknown>) ? 'text-rose-600' : ''}`}>
                  {k === 'modeName'
                    ...
                    : k === 'ts_swing_video'
                    ? (() => {
                        const urls = getSwingVideoUrls(row as Record<string, unknown>, env)
                        if (!urls.length) return <span className="text-rose-600">N/A</span>
                        return (
                          <button className="text-indigo-600 hover:underline px-0" onClick={() => onOpenSwing(urls)}>
                            Click here
                          </button>
                        )
                      })()
                    ...
                    : k === 'ts_driving_range' || k === 'ts_approach' || k === 'td_driving_range' || k === 'td_approach' || k === 'td_putting'
                    ? <div className="max-w-[200px] overflow-x-auto whitespace-nowrap">{formatCell((row as Record<string, unknown>)[k])}</div>
