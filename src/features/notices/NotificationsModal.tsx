...
export default function NotificationsModal({ open, onClose }: Props) {
  ...
  const fetchNotices = async (targetPage: number, append: boolean) => {
    const q = `/api/notices?page=${targetPage}&pageSize=${pageSize}`
    if (!append) {
      setLoading(true)
      setError(null)
      setItems([])
    ...
    const payload = await apiJson<Record<string, unknown> | unknown[]>(q)
  ...
  }, [open, page, loading, loadingMore, hasMore])
    ...
    >
      {loading ? (
        ...
        <div className="space-y-2">
          {items.map((n, idx) => (
            <div
              key={n.id || n._id || String(idx)}
              className="flex items-start gap-3 rounded-xl bg-gray-50 px-3 py-2 border border-gray-100"
            >
          ...
          {loadingMore && (
            <div className="py-2">
              <div className="w-full h-full min-h-12 rounded-md border border-blue-300 p-4">
                <div className="flex animate-pulse space-x-4">
                  <div className="flex-1 space-y-4 py-1">
                    <div className="h-2 rounded bg-gray-200"></div>
                    <div className="h-2 rounded bg-gray-200 w-2/3"></div>
                  </div>
                </div>
              </div>
            </div>
...
function extractNotices(payload: unknown): Notice[] | null {
  const tryArray = (val: unknown): Notice[] | null => {
    ...
    return val.map((x) => {
      ...
      return {
        ...
        time_insert:
          typeof obj['time_insert'] === 'string'
            ? (obj['time_insert'] as string)
            : undefined,
      }
    })
  }
  ...
  if (typeof data === 'object' && data != null) {
    const nested = tryArray(
      (data as Record<string, unknown>)['items'] ||
        (data as Record<string, unknown>)['notices'] ||
