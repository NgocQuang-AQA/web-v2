          ...
          <tbody>
            {users.map((u) => (
              <tr key={u._id} className="hover:bg-gray-50">
                ...
                <td>
                  <span
                    className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-full text-xs font-medium ${u.isActive ? 'bg-green-50 text-green-700' : 'bg-rose-50 text-rose-700'}`}
                  >
                    <span
                      className={`w-1.5 h-1.5 rounded-full ${u.isActive ? 'bg-green-500' : 'bg-rose-500'}`}
                    />
...
function PermissionsTab() {
  ...
  const availableMenus = ['daily', 'ta', 'bug', 'report', 'notes']
              ...
              <div className="flex gap-2 items-start">
                ...
                <div className="flex flex-wrap gap-1">
                  {r.permissions?.map((p) => (
                    <span
                      key={p}
                      className="text-[10px] bg-indigo-50 text-indigo-600 px-1.5 py-0.5 rounded border border-indigo-100"
                    >
                      {p}
                    </span>
                  ))}
              ...
              <div className="flex gap-2 items-start">
                ...
                <div className="flex flex-wrap gap-1">
                  {r.menus?.map((m) => (
                    <span
                      key={m}
                      className="text-[10px] bg-emerald-50 text-emerald-600 px-1.5 py-0.5 rounded border border-emerald-100"
                    >
                      {m}
                    </span>
                  ))}
